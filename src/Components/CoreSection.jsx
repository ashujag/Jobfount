...
import { Briefcase, BarChart3, Users, Award, ChevronLeft, ChevronRight } from 'lucide-react';
              ...
              {services.map((service, index) => {
                ...
                return (
                  <div
                    ...
                    style={{
                      transform: `
                        translateX(${position === 0 ? 0 : position * 180 - 160}px) 
                        translateZ(${isActive ? 0 : -100}px)
                        rotateY(${position === 0 ? 0 : position === 1 ? -8 : position === services.length - 1 ? 8 : 0}deg)
                        scale(${isActive ? (isHovered ? 1.05 : 1) : 0.85})
                      `,
                      opacity: position === 0 ? 1 : position <= 2 ? 1 : 0,
                      zIndex: 10 - position,
                      pointerEvents: position === 0 ? 'auto' : position <= 2 ? 'auto' : 'none',
              ...
              {services.map((_, index) => (
                <button
                  ...
                  className={`
                    ...
                    ${index === activeIndex ? 'w-6 sm:w-8 bg-gray-800' : 'w-1.5 sm:w-2 bg-gray-300'}
                  `}
              ...
              {services.map((service, index) => (
                <button
                  ...
                  className={`
                    ...
                    ${activeIndex === index ? 'bg-gray-50 ring-2 ring-gray-200' : 'hover:bg-gray-50'}
                  `}
                >
                  <div className={`${service.color} w-10 h-10 rounded-lg flex items-center justify-center text-white mb-2`}>
